...
class Bar {
    ...
    update_bar_position({ x = null, width = null }) {
        const bar = this.$bar;
        if(this.task.read_only == null && this.task.read_only == true){ return; }
        if (x) {
    ...
    compute_x() {
        ...
        let x = diff / step * column_width/60;
    ...
    update_label_position() {
        ...
        if (label.getBBox().width > bar.getWidth()) {
            ...
            label.setAttribute('x', bar.getX() - label.getBBox().width - 5);
        } else {
...
class Gantt {
    ...
    setup_gantt_dates() {
        ...
        if (this.view_is(VIEW_MODE.HOUR)){
            ...
            this.gantt_end = date_utils.add(this.gantt_end,25,'hour');
    ...
    get_date_info(date, last_date, i) {
        ...
        const x_pos = {
            ...
            'Hour_upper': -1 * this.options.column_width *24 / 2,
        ...
        if (this.options.view_mode == 'Hour') {
            return {
                upper_text: date_text[`${this.options.view_mode}_upper`],
                lower_text: date_text[`${this.options.view_mode}_lower`],
                upper_x: base_pos.x + x_pos[`${this.options.view_mode}_upper`],
                upper_y: base_pos.upper_y,
                lower_x: base_pos.x + x_pos[`${this.options.view_mode}_lower`] - this.options.column_width,
